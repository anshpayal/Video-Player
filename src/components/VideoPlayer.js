...
const VideoPlayer = ({ video }) => {
  ...
  return (
    <div className="bg-slate-800 border-t-2 border-slate-900 w-full">
      <h1 className="text-white text-xl sm:text-2xl font-semibold mx-5 sm:mx-10 my-5 font-mono">
        Now Playing
      </h1>
      <div className="relative w-11/12 mx-auto sm:w-9/12 sm:mx-10 my-4">
        ...
        <div
          className="absolute top-0 right-0 p-4 flex items-center"
          onMouseEnter={() => setShowSlider(true)}
          onMouseLeave={() => setShowSlider(false)}
        >
          <button
            onClick={toggleMute}
            className="text-white sm:text-xl mr-2 relative z-10"
          >
      ...
      <h1 className="text-center sm:text-left  text-2xl sm:mx-10 my-3 font-mono text-white font-semibold">
        {video.title} - {video.subtitle}
      </h1>
      <h3 className="text-center sm:text-left text-xl sm:mx-10 my-3 font-mono text-white font-semibold">
        Video Description
      </h3>
      <p className="text-center mx-3  sm:text-left sm:mx-10 my-3 font-mono text-gray-400 font-semibold">
        {video.description}
      </p>
    </div>
  );
